...
from class_schedule.solver import _allowed_pattern_types
...
class AllowedPatternTypesTests(unittest.TestCase):
    def test_normal_class_is_standard(self):
        ...
        self.assertEqual(_allowed_pattern_types(item, section), frozenset({"standard"}))
    ...
    def test_hybrid_class_is_standard(self):
        ...
        self.assertEqual(_allowed_pattern_types(item, left), frozenset({"standard"}))
        self.assertEqual(_allowed_pattern_types(item, right), frozenset({"standard"}))
    ...
    def test_four_credit_mwf_row_is_standard(self):
        ...
        self.assertEqual(_allowed_pattern_types(item, mwf), frozenset({"standard"}))
    ...
    def test_four_credit_partial_row_is_four_credit_partial(self):
        ...
        self.assertEqual(
            _allowed_pattern_types(item, t_half), frozenset({"four_credit_partial"})
        )

        r_half = make_section(time_slot="R 9:00am", duration=80)
        item2 = FourCreditClass((mwf, r_half))
        self.assertEqual(
            _allowed_pattern_types(item2, r_half), frozenset({"four_credit_partial"})
        )
    ...
    def test_coreq_three_credit_pairs_are_standard(self):
        ...
        self.assertEqual(_allowed_pattern_types(item2, left2), frozenset({"standard"}))
    ...
    def test_coreq_1113_1110_pair_is_coreq_short(self):
        ...
        item = CoreqClass((left, right))
        self.assertEqual(_allowed_pattern_types(item, left), frozenset({"coreq_short"}))
        self.assertEqual(_allowed_pattern_types(item, right), frozenset({"coreq_short"}))

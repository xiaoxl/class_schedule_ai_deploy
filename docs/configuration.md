...
Every rule selector — `name`, `course`, `subject`, `number`, `section`, `section_prefix`, `room`, `building` — works the same way: an unset selector matches anything; a set selector is one value or a **list**, and the meeting's value must be one of the options (`section_prefix` matches by prefix, the rest exactly). `course` is the `"SUBJECT NUMBER"` shorthand for `subject` + `number` and cannot be combined with either. `subject` scopes a rule to whole departments (`subject = ["MATH", "STAT"]`), `number` to a catalog number across subjects, `section` may stand alone (every `F01`) or narrow a subject/number, and `room`/`building` both feed one location test (a bare building name works in either). Preference rules add a weight — positive rewards, negative penalizes; named rules apply to one instructor, unnamed rules are global.

Constraints are hard rules using the same selectors without a weight (and must still carry `name`, `room`/`building`, and/or `time`). A Friday-noon block for two departments is one rule: `direction = "-"`, `subject = ["MATH", "STAT"]`, and the `time` window. Unknown fields and invalid cross-file references are rejected so mistakes cannot silently alter solver behavior.

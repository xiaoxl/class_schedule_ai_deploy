...
- Recognize a 50/170-minute split-room lab as a `LabClass`, and a lecture
  linked to its lab (shared course number, or catalog siblings) as a
  `LectureLabClass`, before rejecting excess same-course rows. See
  [lab and lecture/lab rules](lecture-lab.md) for matching conditions,
  fixed lab assignments, and baseline metadata.

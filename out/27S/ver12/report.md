# 27S ver12 solve report

Validated solver input: `work/27S/initial/initial.csv` (57 atomic classes, 79 rows)

Initial baseline: `work/27S/initial/initial.csv` (57 atomic classes, 79 rows)

Term changes snapshot: `inputs/27S/changes.toml`; cancelled-course validation passed

Configuration version: `7ad1debafa9f`

Ran 5 independent attempt(s), each with a 45s CP-SAT budget. Selected attempt 5 by lowest worst instructor overload, then lowest solver objective, then lowest reported soft penalty.

## Selected result

- Solver status: feasible
- Solver objective: -600
- Best objective bound: -1665
- Solve time: 45.008 seconds
- Candidate assignments: 6122
- Hard violations: 0
- Soft penalty: 100 (3 findings)
- Worst instructor overload: 2 credit hours
- Remaining placeholder identities: Staff

## Attempt comparison

| Attempt | Status | Objective | Bound | Seconds | Soft | Worst overload | Hard |
|---:|---|---:|---:|---:|---:|---:|---:|
| 1 | feasible | -390 | -1785 | 45.0117 | 285 | 6 | 0 |
| 2 | feasible | -615 | -1795 | 45.008 | 315 | 6 | 0 |
| 3 | feasible | -305 | -1830 | 45.0174 | 470 | 5 | 0 |
| 4 | feasible | -610 | -1575 | 45.0916 | 325 | 6 | 0 |
| 5 | feasible | -600 | -1665 | 45.008 | 100 | 2 | 0 |

## Before and after

| Metric | Before | After |
|---|---:|---:|
| Hard violations | 3 | 0 |
| Soft penalty | 550 | 100 |
| Soft findings | 10 | 3 |
| Worst overload | 1 | 2 |

## Teaching loads

| Instructor | Target | Before | After | Delta |
|---|---:|---:|---:|---:|
| Bain, Leslie M. | 15 | 15 | 15 | +0 |
| Ballard, Kasey L. | 12 | 13 | 13 | +0 |
| Cox, Allie M. | 15 | 16 | 15 | -1 |
| Growns, Landon C. | 15 | 11 | 16 | +5 |
| Jordan, Scott M. | 12 | 12 | 12 | +0 |
| Jordan, Susan M. | 15 | 14 | 15 | +1 |
| King, Jamie L. | 15 | 15 | 15 | +0 |
| Limperis, Thomas G. | 12 | 10 | 14 | +4 |
| Overduin, Matthew D. | 12 | 9 | 12 | +3 |
| Staff | n/a | 23 | 22 | -1 |
| Staff 2 | n/a | 6 | 0 | -6 |
| Staff 3 | n/a | 6 | 0 | -6 |
| Taylor, Teresa L. | 15 | 15 | 15 | +0 |
| Winn, Janet L. | 15 | 15 | 17 | +2 |
| Xiao, Xinli | 12 | 10 | 7 | -3 |
| Yousuf, Marium | 15 | 15 | 17 | +2 |

## Simplified changes from initial

- **MATH 1914-001** instructor: `Cox, Allie M.` -> `Growns, Landon C.`
- **MATH 1914-003** room: `Corley 268` -> `Ross Pendergraft Library 220`
- **MATH 2914-001** time: `MWF 9:00am` -> `MWF 11:00am`
- **MATH 2914-003** instructor: `Yousuf, Marium` -> `Jordan, Susan M.`
- **MATH 2924-002** time: `MWF 12:00pm` -> `MWF 2:00pm`
- **MATH 2924-003** time: `MWF 10:00am` -> `MWF 11:00am`
- **MATH 2934-001** instructor: `Staff` -> `Limperis, Thomas G.`
- **MATH 2934-002** time: `MWF 11:00am` -> `MWF 9:00am`
- **MATH 2934-002** time: `R 11:00am` -> `R 8:00am`
- **MATH 4123-001** instructor: `Staff` -> `Limperis, Thomas G.`
- **MATH 4123-001** time: `MWF 8:00am` -> `MWF 9:00am`
- **MATH 4123-001** room: `Corley 268` -> `Rothwell 312`
- **MATH 4123-H01** instructor: `Staff` -> `Limperis, Thomas G.`
- **MATH 4123-H01** time: `MWF 8:00am` -> `MWF 9:00am`
- **MATH 4123-H01** room: `Corley 268` -> `Rothwell 312`
- **MATH 0803-003** instructor: `Ballard, Kasey L.` -> `King, Jamie L.`
- **MATH 1003-003** instructor: `Ballard, Kasey L.` -> `King, Jamie L.`
- **MATH 0903-001** instructor: `Staff 3` -> `Growns, Landon C.`
- **MATH 0903-001** room: `Corley 104` -> `Corley 102`
- **MATH 1113-001** instructor: `Staff 3` -> `Growns, Landon C.`
- **MATH 0903-002** time: `MWF 8:00am` -> `MWF 2:00pm`
- **MATH 1113-002** time: `TR 8:00am` -> `TR 2:30pm`
- **MATH 1110-003** instructor: `Growns, Landon C.` -> `Winn, Janet L.`
- **MATH 1110-003** time: `MW 2:00pm` -> `MW 12:00pm`
- **MATH 1113-003** instructor: `Growns, Landon C.` -> `Winn, Janet L.`
- **MATH 0803-004** instructor: `Staff 2` -> `Staff`
- **MATH 1003-004** instructor: `Staff 2` -> `Staff`
- **MATH 1003-005** instructor: `Winn, Janet L.` -> `Jordan, Susan M.`
- **MATH 1003-005** room: `Corley 104` -> `Corley 101`
- **MATH 1003-TC2** instructor: `King, Jamie L.` -> `Ballard, Kasey L.`
- **MATH 1113-006** time: `TR 9:30am` -> `TR 2:30pm`
- **MATH 1113-006** room: `Corley 104` -> `Corley 102`
- **MATH 2243-001** instructor: `Overduin, Matthew D.` -> `Bain, Leslie M.`
- **MATH 2243-002** instructor: `Overduin, Matthew D.` -> `Cox, Allie M.`
- **MATH 2703-001** instructor: `Jordan, Susan M.` -> `Ballard, Kasey L.`
- **MATH 2703-001** time: `MWF 10:00am` -> `MWF 8:00am`
- **MATH 2703-001** room: `Corley 101` -> `Rothwell 306`
- **MATH 2703-TC1** instructor: `Jordan, Susan M.` -> `Overduin, Matthew D.`
- **MATH 3033-001** time: `TR 11:00am` -> `TR 8:00am`
- **MATH 3243-001** instructor: `Xiao, Xinli` -> `Overduin, Matthew D.`
- **MATH 3243-002** instructor: `Limperis, Thomas G.` -> `Overduin, Matthew D.`
- **STAT 2163-001** instructor: `King, Jamie L.` -> `Bain, Leslie M.`
- **STAT 2163-TC1** instructor: `Bain, Leslie M.` -> `Yousuf, Marium`
- **STAT 2163-TC2** instructor: `Bain, Leslie M.` -> `Yousuf, Marium`

## Remaining hard violations

- none

## Remaining soft findings

- [under_load] (90) Xiao, Xinli: 7 credit hours is under max_load 12
- [custom_rule] (5) MATH 0803-001: matches a custom dislike rule (weight 5)
- [custom_rule] (5) MATH 1113-002: matches a custom dislike rule (weight 5)
